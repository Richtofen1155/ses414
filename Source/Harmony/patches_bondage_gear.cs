...
namespace rjw {
	...
	static class PATCH_Pawn_ApparelTracker_Wear {
		...
		[HarmonyPrefix]
		static bool prevent_wear_by_gear (Pawn_ApparelTracker __instance, Apparel newApparel)
		{
			var tra = __instance;
		...
		{
			var tra = __instance;
			var def = ((Thing)newApparel).def as bondage_gear_def;
			if ((def != null) &&
			    (newApparel.Wearer == tra.pawn)) { // Check that the apparel was actually worn
	...
	static class PATCH_Pawn_ApparelTracker_Remove {
		...
		static void on_remove (Pawn_ApparelTracker __instance, Apparel ap)
		{
			...
			var def = ((Thing)ap).def as bondage_gear_def;
			if (def != null)
	...
	static class PATCH_Pawn_ApparelTracker_TryDrop {
		...
		{
			var tar = get_target ();
			var pat = typeof(PATCH_Pawn_ApparelTracker_TryDrop).GetMethod ("prevent_gear_drop");
			har.Patch (tar, new HarmonyMethod (pat), null);
		}

		public static bool prevent_locked_apparel_drop (Pawn_ApparelTracker __instance, Apparel ap, ref bool __result)
		{
			if (! ap.has_lock ())
				return true;
			else {
	...
	static class PATCH_WITab_Caravan_Gear_TryEquipDraggedItem {
		...
		[HarmonyPrefix]
		static bool prevent_locked_apparel_conflict (WITab_Caravan_Gear __instance, Pawn p)
		{
			var tab = __instance;
			var dragged_app = get_dragged_item (tab) as Apparel;
			if ((dragged_app != null) && (p.apparel != null) && (dragged_app.has_lock ()))
