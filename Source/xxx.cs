	...
	public static class xxx
	{
		...
		public static bool can_fuck (Pawn pawn)
		{
            if (!xxx.is_animal(pawn)) {
                return (pawn.ageTracker.AgeBiologicalYears >= HugsLibInj.sex_minimum_age) && (pawn.ageTracker.AgeBiologicalYears >= HugsLibInj.sex_free_for_all_age) && (get_sex_ability(pawn) > 0.0f) && (!genitals_blocked(pawn));
		...
		public static bool can_get_raped (Pawn pawn)
		{
            ...
            if (!xxx.is_animal(pawn)) {
                ...
                return (age >= HugsLibInj.sex_minimum_age) && (age >= HugsLibInj.sex_free_for_all_age) && (!genitals_blocked(pawn));
            } else if (xxx.is_animal(pawn) && xxx.config.animals_enabled) {
                return true;
            }
            return false;
